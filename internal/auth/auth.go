// Package auth defines the functionality required to create and verify auth tokens
package auth
...
func CreateJWTAuthToken(googleUserID string, email string, secretAuthKey string, dbClient *database.DBClient) (models.AuthToken, error) {
...
func VerifyJWTAuthToken(token string, secretAuthKey string, dbClient *database.DBClient) (models.TokenPayload, error) {
	...
	if err != nil {
		if errors.Is(err, jwt.ErrTokenExpired) {
			return models.TokenPayload{}, errors.New("token expired")
		}
		return models.TokenPayload{}, err
	}

	if !parsedToken.Valid {
		return models.TokenPayload{}, errors.New("invalid token")
	}
	...
	return models.TokenPayload{GoogleUserID: parsedClaims.GoogleUserID, Email: parsedClaims.Email}, nil
}
